...
from model.cmatt import CMAtten, CM_grounding
import time
...
class VGT(nn.Module):
    def __init__(
        ...
        self.bidirec_att_grounding = CM_grounding()
    ...
    def get_vqa_embedding_rfcpos(self, video, language, language_lens, ans_id, question=None):
        ...
        # print(language.shape, language_lens.shape, question.shape)

        bsize, numc, numf, numr, fdim = video_o.size()
        bsize_lan, len_lan, dim_lan = language.size()  # (bs*ans_n, len_lan, dim_lan)
        ...
        A = F.softmax(A, dim=-1)
        X_o = self.gnn(X, A)
        # (b*numf*numc, numr, dmodel)
        X_o += X

        #########region-level cross-model interaction##############
        X_o_r = X_o.view(bsize, numc, numf, numr, -1)
        X_o_r = X_o_r.mean(dim=-3)
        xlen = numc * numr
        X_o_r = X_o_r.view(bsize, xlen, -1)
        dmodel = X_o_r.shape[-1]
        X_o_r = X_o_r.view(bsize * numc, -1, dmodel)
        #########cross-model interaction##############

        satt_r = self.satt_pool(X_o_r)
        # (b*numc, dmodel)
        X_o_r = torch.sum(X_o_r * satt_r, dim=-2)
        X_o_r = X_o_r.view(bsize, numc, -1)

        X_o_r, attention_clip_arg, cliptopN = self.cm_interaction(X_o_r, numc, language, language_lens, ans_n,
                                                                      ans_id, topk=3)
        ...
        #####cross-model attention#############
        return video, cliptopN, attention_clip_arg

    def grouding_clip(self, cliptopN_argmax):
        cliptopN_argmax = cliptopN_argmax.detach().cpu()
        bsize, topk, dmodel = cliptopN_argmax.size()
        for i in range(bsize):
            for j in range(topk):
                arg = cliptopN_argmax[i][j]
                arg = set(arg.numpy())
                print(arg)
            print('one sample')

    def cm_interaction(self, X, xlen, language, language_lens, ans_n=5, ans_id=None, topk=3):
        bsize = X.shape[0]
        X_len = torch.tensor([xlen] * bsize, dtype=torch.long).to(X.device)
        if ans_n == 1:
            Xatt, attention_mat = self.bidirec_att(X, X_len, language, language_lens.view(-1))
        else:
        ...
        cliptopkN = torch.topk(X, k=topk, dim=1)[0]
        attention_mat = attention_mat.view(bsize, ans_n, xlen, -1).max(dim=1)[0]
        attention_arg = attention_mat.mean(dim=2).topk(dim=1, k=3, largest=True)[1]
        return X, attention_arg, cliptopkN

    def cm_interaction_QA(self, X, xlen, language, language_lens, topk=3):
        bsize = X.shape[0]
        X_len = torch.tensor([xlen] * bsize, dtype=torch.long).to(X.device)
        ...
        Xatt += X
        cliptopkN = torch.topk(Xatt, k=topk, dim=1)[0]
        attention_arg = attention_mat.mean(dim=2).topk(dim=1, k=2, largest=False)[1]
        return X, attention_arg, cliptopkN
    ...
    def forward(
        ...
        if mode == "vqa":
            ...
            if self.Q != 0:
                ...
                video_proj_, clipwise_nod2e, attention_clip_arg = self.get_vqa_embedding_rfcpos(video, answer_w, seq_len,
                                                                                               ans_id, question_w)
                video_proj = self.position_v(video_proj_)
                ...
                video_proj_, clipwise_node, attention_clip_arg = self.get_vqa_embedding_rfcpos(video, answer_w, seq_len, ans_id)
                video_proj = self.position_v(video_proj_)
                attended_v = self.mmt(x=video_proj, attn_mask=video_mask)[0]
                global_feat = attended_v.mean(dim=1)
                fusion_proj = self.vqproj(global_feat)
            ...
            if answer is not None:
                return fusion_proj, answer_g, clipwise_node, attention_clip_arg
